...
def generate_reviews(products, review_count):
    """
    리뷰 생성
    :param products: [{"product_no": "1234", "category": "액상", "name": "상품명"}, ...]
    ...
    for i in range(review_count):
        ...
        # 카테고리에 맞는 리뷰 템플릿 선택
        category = product.get("category", "액상")
        ...
        review = {
            ...
            "rating": random.choice([4, 5]),  # 4-5점 랜덤
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python generate_reviews.py '<JSON_DATA>'")
        print("Example: python generate_reviews.py '{\"products\": [{\"product_no\": \"1234\", \"category\": \"액상\"}], \"count\": 10, \"output\": \"output.xlsx\"}'")
        sys.exit(1)
